...
//     					'rightAttribute' => 'rgt',
    ...
    public static function getTree($node_id = 0){
    	...
    	foreach ($row as $r)    		
    		$return[$r->id]= str_repeat('--', $r->depth). ' ' . $r['slide_detail']['name'];
    		return $return;
    }
    
    public function getSlide_detail()
    {
    	return $this->hasOne(SlidedetailsModel::className(), ['cate_id' => 'id']);
    }

    public function getSlide_detail_update()
    {
    	return $this->hasMany(SlidedetailsModel::className(), ['cate_id' => 'id']);
    }
